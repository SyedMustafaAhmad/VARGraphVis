...
{
    internal sealed class GradientLinkFactory  : LinkFactoryBase
    {
        ...
        public override LinkBase CreateInstance(LinkDto link, Transform linksContainer, Dictionary<string, NodeBase> idToNode)
        {
            ...
            if (linkComponent != null)
            {
                ...
                {
                    linkComponent.SetColorStart(linkColorByValue[sourceNode.group]);
                    linkComponent.SetColorEnd(linkColorByValue[targetNode.group]);
                }
                else
                {
